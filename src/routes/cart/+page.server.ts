...
export const actions: Actions = {
	default: async ({ request }) => {
		...
		const couponCode = asString(formData, 'couponCode');
		...
		const noteLines = [notes, couponCode ? `Coupon code: ${couponCode}` : ''].filter(Boolean);
		const persistedNotes = noteLines.join('\n');
		const messageLines = [
